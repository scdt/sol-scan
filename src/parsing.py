...
def get_parser(tool):
    ...
    if key not in tool_parsers:
        try:
            ...
            fn = os.path.join(src.cfg.TOOLS_HOME, tid, tool["parser"])
            spec = importlib.util.spec_from_file_location(modulename, fn)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            tool_parsers[key] = module
        except Exception as e:
            raise SolScanError(f"Cannot load parser for {tid}/{tmode}\n{e}")
...
def parse(task_log, tool_log, tool_output):
    ...
    try:
        ...
        for finding in findings:
            ...
            # irrelevant for SolScan, but may be relevant for programs further down the line
        ...
        raise SolScanError(f"Parsing of results failed\n{e}")
