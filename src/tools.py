...
import src.io
import src.cfg
from src.exceptions import SolScanError, InternalError
...
class Tool():
    ...
    def __init__(self, cfg):
            ...
            if v is not None:
                if k in ("solc"):
                    try:
                        ...
                        raise SolScanError(f"Tool: value of attribute '{k}' is not a Boolean.\n{cfg}")
                elif k in ("cpu_quota"):
                    try:
                        v = int(v)
                        assert v >= 0
                    except:
                        raise SolScanError(f"Tool: value of attribute '{k}' is not an integer>=0.\n{cfg}")
                elif k in ("mem_limit"):
                    try:
                        v = str(v).replace(" ", "")
                        if v[-1] in "kKmMgG":
                        ...
                        raise SolScanError(f"Tool: value of attribute '{k}' is not a valid memory specifcation.\n{cfg}")
                else:
                    try:
                        v = str(v)
                    except:
                        raise SolScanError(f"Tool: value of attribute '{k}' is not a string.\n{cfg}")
        ...
        if not self.image:
            raise SolScanError(f"Tool {self.id}/{self.mode}: no image specified")
        extras = set(cfg.keys()).difference(FIELDS)
        if extras:
            raise SolScanError(f"Tool {self.id}/{self.mode}: extra field(s) {', '.join(extras)}")
        if not self._command and not self._entrypoint:
            raise SolScanError(f"Tool {self.id}/{self.mode}: neither command nor entrypoint specified.")
        if not self.parser:
            self.parser = src.cfg.TOOL_PARSER
        if self.bin:
            self.absrcin = os.path.join(src.cfg.TOOLS_HOME, self.id, self.bin)
    ...
    def command(self, filename, timeout, bin):
        try:
            ...
            raise SolScanError(f"Unknown variable '{e}' in command of tool {self.id}/{self.mode}")
    ...
    def entrypoint(self, filename, timeout, bin):
        try:
            ...
            raise SolScanError(f"Unknown variable '{e}' in entrypoint of tool {self.id}/{self.mode}")
...
def load(ids, tools=[], seen=set()):
    ...
    for id in ids:
        if id in seen:
            continue
        seen.add(id)
        toolpath = os.path.join(src.cfg.TOOLS_HOME, id)
        fn = os.path.join(toolpath, src.cfg.TOOL_CONFIG)
        cfg = src.io.read_yaml(fn)
        ...
        for mode in ("solidity", "bytecode", "runtime"):
            ...
            if not isinstance(cfg[mode], dict):
                raise SolScanError(f"Tool {id}/{mode}: key/value mapping expected.")
            cfg_copy.update(cfg[mode])
            tools.append(Tool(cfg_copy))
        if not found:
            raise SolScanError(f"{fn}: needs one of the attributes 'alias', 'solidity', 'bytecode', 'runtime'")
    return tools
...
def info_finding(tool_id, fname):
    if tool_id not in info_findings:
        try:
            fn = os.path.join(src.cfg.TOOLS_HOME, tool_id, src.cfg.TOOL_FINDINGS)
            info_findings[tool_id] = src.io.read_yaml(fn)
        except Exception:
