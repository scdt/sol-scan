...
def main():
    ...
    for r in sorted(results):
        ...
        try:
            task_log = src.io.read_json(os.path.join(r, src.cfg.TASK_LOG))
        except Exception as e:
            print(f"Cannot read task log: {e}")
            continue
        try:
            parser_output = src.io.read_json(os.path.join(r, src.cfg.PARSER_OUTPUT))
        except Exception as e:
...
def data2csv(task_log, parser_output, postgres, fields):
    csv = {
        ...
        "findings": sorted({src.utils.str2label(f["name"])
